...
```bash
git clone https://github.com/michalstrnadel/slevomat-ai-skills.git /tmp/sas && cp -r /tmp/sas/slevomat-design-principles ~/.claude/skills/ && rm -rf /tmp/sas
```

Po instalaci: restart Claude Code, napiš `design principy` nebo `design check` v chatu.

**Claude.ai (web):**

Stáhni [`slevomat-design-principles.zip`](./slevomat-design-principles.zip) → Skills panel → **+** → **Upload a skill** → vyber ZIP.
...
1. Naklonuj repo: `git clone <repo-url> && cd slevomat-design-principles`
2. Spusť Claude Code: `claude` ve složce repa
3. V chatu napiš: `design principy` + popis nebo přilož screenshot

### Použití mimo Claude Code

#### Claude.ai (s nahráním do Skills)

1. Stáhni [`slevomat-design-principles.zip`](./slevomat-design-principles.zip) (klik na soubor → tlačítko **Download** vpravo nahoře)
2. V Claude.ai → Skills panel → **+** → **Create skill** → **Upload a skill**
3. Vyber stažený ZIP
4. Skill se objeví v Personal skills, aktivuje se napsáním `design principy` nebo `design check` v konverzaci
...
> "Načti SKILL.md a design-principles.md z https://github.com/michalstrnadel/slevomat-design-principles a aplikuj skill na tenhle screenshot."
